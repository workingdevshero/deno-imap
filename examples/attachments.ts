#!/usr/bin/env -S deno run --allow-net --allow-env --env-file=.env
...
try {
  ...
  const fetchRange = `${Math.max(1, messageCount - fetchCount + 1)}:${messageCount}`;
  ...
  for (const message of messagesWithAttachments) {
    ...
    if (message.bodyStructure) {
      ...
      for (const attachment of attachmentInfo) {
        console.log(`- ${attachment.filename} (${attachment.type}/${attachment.subtype}, ${attachment.size} bytes)`);
        console.log(`  Section: ${attachment.section}, Encoding: ${attachment.encoding}`);
      }
      
      // Fetch the first attachment if there are any
      if (attachmentInfo.length > 0) {
        const firstAttachment = attachmentInfo[0];
        console.log(`\nFetching attachment "${firstAttachment.filename}"...`);
        
        try {
          // Fetch the attachment using the client's fetch method with bodyParts
          const fetchResult = await client.fetch(`${message.seq}`, {
            bodyParts: [firstAttachment.section],
          });
          
          if (fetchResult.length > 0 && fetchResult[0].parts && fetchResult[0].parts[firstAttachment.section]) {
            const attachmentData = fetchResult[0].parts[firstAttachment.section];
            console.log(`Successfully fetched attachment (${attachmentData.size} bytes)`);
            
            // For text-based attachments, show a preview
            if (
              firstAttachment.type === "TEXT" || 
              (firstAttachment.type === "APPLICATION" && 
               (firstAttachment.subtype === "JSON" || 
                firstAttachment.subtype === "XML" || 
                firstAttachment.subtype === "JAVASCRIPT"))
            ) {
              const decoder = new TextDecoder();
              const content = decoder.decode(attachmentData.data as Uint8Array);
              console.log("\nPreview of attachment content:");
              console.log(content.substring(0, 200) + (content.length > 200 ? "..." : ""));
            } else {
              console.log(`Binary attachment of type ${firstAttachment.type}/${firstAttachment.subtype} fetched successfully.`);
              console.log(`To save this attachment, you would write the binary data to a file.`);
            }
          } else {
            console.log("Failed to fetch attachment data. The attachment might be empty or inaccessible.");
          }
        } catch (error) {
          console.error("Error fetching attachment:", error instanceof Error ? error.message : String(error));
        }
      }
    }
  }
  ...
  if (messagesWithAttachments.length === 0) {
    ...
    console.log("Try sending yourself an email with an attachment to test this example.");
  }
